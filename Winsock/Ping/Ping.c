...
{
	unsigned long checksum = 0; /* 设置校验和初值为0 */
	while (buf_len > 1)
	{
		checksum += *buf++; /* 求各个16位数字之和 */
	...
	checksum = (checksum >> 16) + (checksum & 0xffff); /* 高位与低位想加 */
	checksum += (checksum >> 16);	/* 加上进位 */

	return (unsigned short)(~checksum);	/* 取反得到检验和 */
